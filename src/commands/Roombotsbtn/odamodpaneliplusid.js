...
export const data = {
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        if(interaction.isButton()){
            ...
            .setTitle("Kullanıcı Id Gir")
            .setComponents(
                new MessageActionRow()
                    .setComponents(
                        new TextInputComponent()
                            .setCustomId("Kullanıcı Id")
                            .setLabel("ID:")
                            .setMinLength(8)
                            .setMaxLength(20)
                            .setPlaceholder("Kullanici Id")
                            .setRequired(true)
                            .setStyle("SHORT")
                    )
            )
        ...
        try {
            ...
            if(insertmod === "odaat"){
                ...
                if (interaction.deferred) interaction.editReply({ embeds: [ embed(`Başarıyla <@${SelectedUserId}> kişinin ses paneli iznini açtınız.`, "YELLOW", `Kanalda Ses Paneli İzni Açıldı`) ], components: [],ephemeral: true })
            }
