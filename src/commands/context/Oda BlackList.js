...
export const data = {
    name: "Oda BlackList",
    description:"Oluşturduğunuz kanalda kullanıcıyı blacklist ekler",
    cooldown: 10,   
    ...
    async execute(interaction, foundchannedid = "", foundchannedownerid = "") {  
        ...
        .then(async channel  => {
            ...
            await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <@${target.id}> kişisi <#${channel.id}> kanalında \`blacklist\` eklendi.`, "BLACK", `Blacklist Eklendi (${channel.id})`) ] })
            if(interaction.channel.type === "GUILD_TEXT") interaction.editReply({ embeds: [ embed(`Başarıyla <@${target.id}> kişinini \`${channel.name}\` kanalında \`Blacklist\` eklediniz.`, "BLACK", `Blacklist Eklendi`) ], ephemeral: true })
        })
        .catch(error => {
            //console.error(error)
