...
export const data = {
    name: "odaunmod",
    description:"Oluşturduğunuz kanalda belirlediğin kişiyi unMod yapar.",
    cooldown: 30,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        try { 
            ...
            command = {
                data:{
                    privroomcommands:"roomcommenddatamoddel",
                    privroomchannelid: channel.id,
                    privroomdeletemodid: selectedMember
                }
            }
            ...
            if (interaction.type !== "DEFAULT" && interaction.deferred) await interaction.editReply({embeds: [embed(`Başarıyla <@${selectedMember}> kişisi kanalda unMod yapıldı.`,"RED","Kanalda unMod Yap")],components: [], ephemeral: true })
            const LogForChannel = await client.channels.fetch(process.env.roomlogchannel);
            await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <@${selectedMember}> kişisi \`${channel.name}\` kanalında unMod yapıldı.`, "RED", `Kanalda unMod Yapıldı (${channel.id})`) ]})
