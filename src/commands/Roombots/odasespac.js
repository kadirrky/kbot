...
export const data = {
    name: "odasespac",
    description:"Oluşturduğunuz kanalda ses paneli iznini açar.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        try {  
            ...
            .then(async channel  => {
                ...
                await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanalında ses paneli izni açıldı.`, "YELLOW", `Kanalda Ses Paneli Açıldı (${channel.id})`) ] })
                if(interaction.type !== "DEFAULT" && interaction.deferred){            
                    interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalında ses paneli iznini açtınız.`, "YELLOW", `Kanalda Ses Paneli İzni Açıldı`) ], ephemeral: true })
                }  
