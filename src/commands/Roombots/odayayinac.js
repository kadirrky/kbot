...
export const data = {
    name: "odayayinac",
    description:"Oluşturduğunuz kanalda kullanıcılara yayın-kamera iznini açar.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        try {  
            ...
            .then(async channel  => {
                ...
                await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanalında kullanıcılara yayın-kamera izni açıldı.`, "YELLOW", `Kanalda Yayın İzni Açıldı (${channel.id})`) ] })
                if(interaction.type !== "DEFAULT" && interaction.deferred){            
                    interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalında kullanıcılara yayın-kamera iznini açtınız.`, "YELLOW", `Kanalda Yayın İzni Açıldı`) ], ephemeral: true })
                }  
