...
export const data = {
    name: "odayayinkapat",
    description:"Oluşturduğunuz kanalda kullanıcılara yayın-kamera iznini kapatır.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        try {  
            ...
            .then(async channel  => {
                ...
                await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanalında kullanıcılara yayın-kamera izni kapatıldı.`, "BLUE", `Kanalda Yayın İzni Kapatıldı (${channel.id})`) ] })
                if(interaction.type !== "DEFAULT" && interaction.deferred){            
                    interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalında kullanıcılara yayın-kamera iznini kapattınız.`, "BLUE", `Kanalda Yayın İzni Kapatıldı`) ], ephemeral: true })
                }  
