...
export const data = {
    name: "odakilitac",
    description:"Oluşturduğunuz kanalının kilidini açar.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        try {  
            ...
            .then(async channel  => {
                ...
                await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanal kilidi açıldı.`, "GREEN", `Kanal Kilidi Açıldı (${channel.id})`) ] })
                if(interaction.type !== "DEFAULT" && interaction.deferred){
                    interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalının kilidini açtınız.`, "GREEN", `Kanal Kilidi Açıldı`) ], ephemeral: true })
                }  
