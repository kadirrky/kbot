...
export const data = {
    name: "odaetkkapat",
    description:"Oluşturduğunuz kanalda kullanıcılara etkinlik iznini kapatır.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        try {  
            ...
            .then(async channel  => {
                ...
                await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanalında kullanıcılara etkinlik izni kapatıldı.`, "BLUE", `Kanalda Etkinlik İzni Kapatıldı (${channel.id})`) ] })
                if(interaction.type !== "DEFAULT" && interaction.deferred){            
                    interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalında kullanıcılara etkinlik iznini kapattınız.`, "BLUE", `Kanalda Etkinlik İzni Kapatıldı`) ], ephemeral: true })
                }  
