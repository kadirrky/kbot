...
export const data = {
    name: "odakilitle",
    description:"Oluşturduğunuz kanalı kilitler.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        .then(async channel  => {
            ...
            await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanalı kilitlendi.`, "RED", `Kanal Kilitlendi (${channel.id})`) ] })
            if(interaction.type !== "DEFAULT" && interaction.deferred){
                    interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalını kilitlediniz.`, "RED", `Kanal Kilitlendi`) ], ephemeral: true }) 
            }
