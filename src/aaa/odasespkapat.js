...
export const data = {
    name: "odasespkapat",
    description:"Oluşturduğunuz kanalda yeni gelen kullanıcılara mikrofon iznini kapatır.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        .then(async channel  => {
            ...
            await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanalında yeni gelen kullanıcılara mikrofon izni kapatıldı.`, "BLUE", `Kanalda Mikrofon Kapatıldı (${channel.id})`) ] })
            if(interaction.type !== "DEFAULT" && interaction.deferred){            
                interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalında yeni gelen kullanıcılara mikrofon iznini kapattınız.`, "BLUE", `Kanalda Mikrofon Kapatıldı`) ], ephemeral: true })
            }  
            if(interaction.type !== "DEFAULT" && interaction.isButton()) privrooms_panelupdate(interaction)
        })
        .catch(error => {
            console.error(error)
            if (interaction.deferred) interaction.editReply({ embeds: [ embed(`Bu komut \`${data.name}\` kullanılırken hata oluştu.`,"RED",) ], ephemeral: true })
        })
