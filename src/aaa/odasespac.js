...
export const data = {
    name: "odasespac",
    description:"Oluşturduğunuz kanalda yeni gelen kullanıcılara mikrofon iznini açar.",
    cooldown: 5,
    ...
    async execute(interaction, foundchannedid = "") {  
        ...
        .then(async channel  => {
            ...
            await LogForChannel.send({ embeds: [ embed(`<@${member.id || interaction.member.id}> tarafından <#${channel.id}> kanalında yeni gelen kullanıcılara mikrofon izni açıldı.`, "YELLOW", `Kanalda Mikrofon İzni Açıldı (${channel.id})`) ] })
            if(interaction.type !== "DEFAULT" && interaction.deferred){            
                interaction.editReply({ embeds: [ embed(`Başarıyla \`${channel.name}\` kanalında yeni gelen kullanıcılara mikrofon iznini açtınız.`, "YELLOW", `Kanalda Mikrofon İzni Açıldı`) ], ephemeral: true })
            }  
